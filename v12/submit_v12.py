"""v12 production: cleaned floor top-5 + MetFrag block 6-12 + GBM fills.
...
def main():
    ...
    for mi, (mol, spectra) in enumerate(test.groupby("molecule_id")):
        ...
        for _, t in _pool.iterrows():
            _b = 0.0
            for _, r in spectra.iterrows():
                _e = entropy_similarity(r["ms2_mzs"], r["ms2_normalized_intensities"],
                         t["ms2_mzs"], t["ms2_normalized_intensities"])
                if _e > _b:
                    _b = _e
        ...
        # metfrag block 6-12: independent isomer evidence (3-seed winner)
        _mf_rank = sorted(((r[6], s) for r, s in zip(_rows, _order)), reverse=True)
        seen = set(top5)
        out = list(top5)
        for _, s in _mf_rank:
            if s in seen:
            ...
            if len(out) == 12:
                break
        out += [s for _, s in scored if not (s in seen or seen.add(s))][:25 - len(out)]
