"""v13 production: v10 + tautomer-canonical (inchikey14) dedup.
Writes submission.csv. One isolated change from v10.
"""
...
def main():
    ...
    # inchikey14 maps for tautomer dedup (metric is stereo-blind)
    _key = dict(zip(train["normalized_smiles"], train["inchikey14"]))
    try:
        _cc = pd.read_parquet(f"{FP}/coco_keys.parquet")
        for s, k in zip(_cc["canonical_smiles"], _cc["key14"]):
            _key.setdefault(s, k)
    except Exception:
        pass
    ...
    for mi, (mol, spectra) in enumerate(test.groupby("molecule_id")):
        ...
        seen = set(top5)
        seen_keys = set(_key.get(s, s) for s in top5)
        out = list(top5)
        for _, s in scored:
            if s in seen:
                continue
